/**
 * The Merchant of Echoes — Bespoke Concept Web Application
 * Audio Engine, Interactive Lyric Stage, 4-Act Narrative Filter,
 * 7-Pillar Epistemic Matrix, Exhibits Lightbox & Accessibility Controls
 */
...
(function() {
  ...
  const elements = {
    ...
    stageKeyBpmBadge: document.getElementById('stage-key-bpm-badge'),
  ...
  function loadPreferences() {
    try {
      const savedVolume = localStorage.getItem('merchant_volume');
      if (savedVolume !== null) {
      ...
      if (localStorage.getItem('merchant_contrast') === 'true') {
        state.highContrast = true;
        document.body.classList.add('high-contrast');
      }

      if (localStorage.getItem('merchant_dyslexic') === 'true') {
        state.dyslexicFont = true;
        document.body.classList.add('dyslexic-font');
      }

      if (localStorage.getItem('merchant_motion') === 'true') {
        state.reducedMotion = true;
        document.body.classList.add('reduced-motion');
      }

      const savedFontStep = localStorage.getItem('merchant_font_step');
  ...
  function applyFontSize() {
    ...
    localStorage.setItem('merchant_font_step', state.fontSizeStep);
  }
  ...
  function init() {
    ...
    console.log("The Merchant of Echoes initialized with", state.albumData.tracks.length, "tracks.");
  }
        ...
        <div class="flex items-center gap-3 min-w-0">
          ...
          <div class="min-w-0">
            ...
            <div class="flex items-center gap-2 text-xs text-stone-400 mt-0.5">
              ...
              <span>•</span>
              <span class="truncate hidden sm:inline text-stone-400 font-mono text-[11px]">${track.key}</span>
            </div>
          </div>
        </div>
  ...
  function loadTrack(index, autoPlay = false) {
    ...
    if (elements.stageKeyBpmBadge) elements.stageKeyBpmBadge.textContent = `${track.key} • ${track.bpm}`;
    ...
    if (elements.stickyTrackMeta) elements.stickyTrackMeta.textContent = `Track ${track.number} • Act ${track.act_number} • ${track.key}`;
      ...
      row.innerHTML = `
        <td class="font-display font-bold text-amber-400 whitespace-nowrap">
          <div class="flex items-center gap-2">
            <span class="w-6 h-6 rounded-full bg-amber-950/60 border border-amber-600/50 flex items-center justify-center text-xs font-mono text-amber-300">${p.number}</span>
            <span>${p.name}</span>
          </div>
        </td>
        <td class="text-xs sm:text-sm text-stone-300">${p.core_concept}</td>
        <td class="text-xs sm:text-sm text-stone-300">${p.mechanism}</td>
        <td class="text-xs sm:text-sm text-stone-400">${p.societal_consequence}</td>
      `;
  ...
  function setupEventListeners() {
    ...
    if (elements.stickyProgress) {
      elements.stickyProgress.addEventListener('input', (e) => {
        if (window.CastManager && window.CastManager.isConnected()) {
          ...
          const parts = (t && t.duration) ? t.duration.split(':') : ['3', '56'];
          const totalSec = parts.length === 2 ? parseInt(parts[0], 10) * 60 + parseInt(parts[1], 10) : 236;
    ...
    if (elements.stickyVolume) {
      elements.stickyVolume.addEventListener('input', (e) => {
        ...
        state.isMuted = (val === 0);
        updateMuteIcon();
        try { localStorage.setItem('merchant_volume', val); } catch(e) {}
      });
    }
    ...
    if (elements.contrastToggleBtn) {
      elements.contrastToggleBtn.addEventListener('click', () => {
        ...
        try { localStorage.setItem('merchant_contrast', state.highContrast); } catch(e) {}
      });
    }
    ...
    if (elements.dyslexicToggleBtn) {
      elements.dyslexicToggleBtn.addEventListener('click', () => {
        ...
        try { localStorage.setItem('merchant_dyslexic', state.dyslexicFont); } catch(e) {}
      });
    }
    ...
    if (elements.motionToggleBtn) {
      elements.motionToggleBtn.addEventListener('click', () => {
        ...
        try { localStorage.setItem('merchant_motion', state.reducedMotion); } catch(e) {}
      });
    }
