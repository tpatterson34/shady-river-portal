/**
 * Letter to the New Gods — Interactive Concept App Engine
 * Audio streaming, lyrics switcher, exhibits explorer, and release voting.
 */

(function () {
  'use strict';

  let data = window.NEW_GODS_DATA || null;
  let tracks = [];
  ...
  function init() {
    if (!data && typeof window.NEW_GODS_DATA !== 'undefined') {
      data = window.NEW_GODS_DATA;
    }
  ...
  function renderTrackList() {
    ...
    filtered.forEach(function (t) {
      ...
      row.className = 'track-row flex items-center justify-between p-3.5 sm:p-4 rounded-xl border transition-all cursor-pointer ' +
        (isCurrent ? 'active bg-cyan-950/25 border-cyan-500/50 text-white' : 'bg-[#0e131a] border-stone-800 hover:bg-stone-900 hover:border-stone-700 text-stone-300');
      row.setAttribute('data-track-index', idx);
      row.setAttribute('role', 'button');
      row.setAttribute('tabindex', '0');

      const iconHtml = (isCurrent && isPlaying)
        ? '<i class="fa-solid fa-volume-high text-cyan-400 animate-pulse"></i>'
      ...
      row.innerHTML = [
        ...
        '  <span class="w-7 text-center font-mono text-xs ' + (isCurrent ? 'text-cyan-400 font-bold' : 'text-stone-500') + '">',
        '    ' + iconHtml,
        '  </span>',
        '  <div class="min-w-0">',
        '    <div class="track-title font-display font-bold text-sm truncate ' + (isCurrent ? 'text-cyan-300' : 'text-stone-200') + '">',
        '      ' + t.title,
        '    </div>',
        ...
        '    ' + t.leadArchetype,
        '  </span>',
        '  <button class="w-8 h-8 rounded-full ' + (isCurrent && isPlaying ? 'bg-cyan-500 text-stone-950 font-bold' : 'bg-stone-800 text-stone-300 hover:text-white') + ' flex items-center justify-center text-xs transition-colors" aria-label="Play ' + t.title + '">',
    ...
    actFilterBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        actFilterBtns.forEach(function (b) {
          b.classList.remove('bg-cyan-600', 'text-stone-950', 'font-bold');
          b.classList.add('bg-stone-900', 'text-stone-300');
        });
        btn.classList.add('bg-cyan-600', 'text-stone-950', 'font-bold');
  ...
  function submitVote() {
    localStorage.setItem('voted_letter-to-the-new-gods', 'true');
    checkVoteStatus();
    closeVoteModal();
  }

  function checkVoteStatus() {
    const voted = localStorage.getItem('voted_letter-to-the-new-gods');
    if (voteHeroBtn) {
      if (voted) {
        voteHeroBtn.innerHTML = '<i class="fa-solid fa-check text-emerald-400 mr-2"></i><span>Release Prioritized</span>';
        voteHeroBtn.classList.remove('bg-cyan-600', 'hover:bg-cyan-500', 'text-stone-950');
