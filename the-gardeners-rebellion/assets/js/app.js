/**
 * Album XX: The Gardener's Rebellion
 * Bespoke Interactive Concept Experience & Streaming Jukebox Engine
...
(function () {
  ...
  function init() {
    ...
    if (data && data.tracks && data.tracks.length > 0) {
      ...
      fetch('data/album-data.json')
        ...
        .catch(function (err) {
          console.error('Failed to load The Gardener\'s Rebellion album data:', err);
        });
    }
  }
  ...
  function renderTrackList() {
    ...
    filtered.forEach(function (t) {
      ...
      row.className = 'track-row flex items-center justify-between p-3 sm:p-3.5 rounded-xl border transition-all cursor-pointer ' +
        (isCurrent ? 'active bg-emerald-950/40 border-emerald-500/60 text-white shadow-lg shadow-emerald-950/20' : 'bg-[#111611] border-[#232c23] hover:bg-[#171e17] hover:border-emerald-700/60 text-stone-300');
      row.setAttribute('data-track-index', idx);
      row.setAttribute('role', 'button');
      row.setAttribute('tabindex', '0');

      const iconHtml = (isCurrent && isPlaying)
        ? '<i class="fa-solid fa-volume-high text-emerald-400 animate-pulse"></i>'
      ...
      row.innerHTML =
        '<div class="flex items-center gap-3 min-w-0 pr-2">' +
          '<div class="w-10 h-10 rounded-lg overflow-hidden shrink-0 border border-stone-800 bg-stone-900 hidden sm:block">' +
            '<img src="assets/images/album-art.webp" alt="" class="w-full h-full object-cover">' +
          '</div>' +
          '<div class="font-mono text-xs font-bold text-emerald-400/90 w-6 shrink-0 text-center">' + iconHtml + '</div>' +
          '<div class="min-w-0">' +
            '<div class="font-display font-semibold text-sm truncate ' + (isCurrent ? 'text-emerald-300 font-bold' : 'text-stone-200') + '">' +
              t.title +
            '</div>' +
            '<div class="flex items-center gap-2 text-[11px] font-mono text-stone-400 truncate mt-0.5">' +
              '<span>' + t.tempo + '</span>' +
              '<span>&bull;</span>' +
              '<span class="text-emerald-400/80 truncate">' + t.archetype + '</span>' +
            '</div>' +
          '</div>' +
        '</div>' +
        '<div class="flex items-center gap-3 shrink-0">' +
          '<span class="font-mono text-xs text-stone-400 hidden xs:inline">' + t.duration + '</span>' +
          '<button class="row-play-btn w-8 h-8 rounded-full ' + (isCurrent ? 'bg-emerald-500 text-stone-950' : 'bg-stone-900 text-stone-300 hover:text-white') + ' flex items-center justify-center text-xs transition-colors" aria-label="Play ' + t.title + '">' +
  ...
  function selectTrack(index, autoplay) {
    ...
    if (noteBtn) {
      noteBtn.onclick = () => {
        ...
        if (typeof window.openBardNoteModal === 'function') {
          window.openBardNoteModal(sTitle, "The Gardener's Rebellion", sNum);
        }
      };
    ...
    if (trackArtEl) trackArtEl.src = 'assets/images/' + (t.art_file || 'album-art.webp');
    ...
    if (playerBarArt) playerBarArt.src = 'assets/images/' + (t.art_file || 'album-art.webp');
  ...
  function bindAudioEvents() {
    ...
    if (volumeSlider) {
      volumeSlider.addEventListener('input', function (e) {
        ...
        localStorage.setItem('tgr_volume', val);
      });
    }
    ...
    if (muteBtn) {
      muteBtn.addEventListener('click', function () {
        ...
        if (audio.muted) {
          ...
          const savedVol = parseFloat(localStorage.getItem('tgr_volume') || 0.85);
  ...
  function bindUIEvents() {
    ...
    actFilterBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        btn.className = 'px-3.5 py-1.5 rounded-lg bg-emerald-500 text-stone-950 font-bold transition-colors';
    ...
    exhibitTabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        tab.className = 'exhibit-tab-btn active px-4 py-3 border-b-2 border-emerald-500 text-emerald-400 font-bold transition-colors';
    ...
    if (confirmVoteBtn) {
      confirmVoteBtn.addEventListener('click', function () {
        ...
        localStorage.setItem('tgr_vote_format', formatVal);
        confirmVoteBtn.textContent = 'Ballot Cast! Thank You';
        confirmVoteBtn.disabled = true;
        confirmVoteBtn.classList.replace('bg-emerald-500', 'bg-emerald-600');
  ...
  function initA11yToolbar() {
    ...
    bindToggle(toggleContrastBtn, 'tgr_a11y_contrast', 'high-contrast');
    bindToggle(toggleFontBtn, 'tgr_a11y_font', 'large-font');
    bindToggle(toggleUnderlineBtn, 'tgr_a11y_links', 'underlined-links');
    bindToggle(toggleMotionBtn, 'tgr_a11y_motion', 'reduced-motion');
  }

  // --- VOLUME & VOTE PREFERENCES ---
  function loadPreferences() {
    const savedVol = localStorage.getItem('tgr_volume');
    if (savedVol !== null) {
    ...
    if (localStorage.getItem('tgr_vote_format')) {
      if (confirmVoteBtn) {
        confirmVoteBtn.textContent = 'Ballot Cast! Thank You';
        confirmVoteBtn.disabled = true;
        confirmVoteBtn.classList.replace('bg-emerald-500', 'bg-emerald-600');
