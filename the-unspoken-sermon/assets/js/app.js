/* The Unspoken Sermon - Application Engine */
document.addEventListener('DOMContentLoaded', () => {
  ...
  const deckParable = document.getElementById('deck-parable');
  ...
  function renderTracklist() {
    ...
    const filtered = album.tracks.filter(t => {
      ...
      const matchesSearch = !q || (
        ...
        t.parable_reference.toLowerCase().includes(q) ||
    ...
    if (filtered.length === 0) {
      tracklistContainer.innerHTML = '<div style="padding: 24px; text-align: center; color: var(--text-dim);">No matching parables found.</div>';
      return;
    }
    ...
    filtered.forEach(track => {
      ...
      row.innerHTML = `
        ...
        <div class="track-info">
          ...
          <div class="track-parable">${track.parable_reference}</div>
        </div>
  ...
  function loadTrack(index, autoPlay = false) {
    ...
    deckParable.textContent = track.parable_reference;
  ...
  // Render Theological Matrix Table
  function renderMatrix() {
    if (!matrixTbody || !album.theological_matrix) return;
    matrixTbody.innerHTML = '';
    album.theological_matrix.forEach(row => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>
          <span class="matrix-dimension">${row.dimension}</span>
          <span class="matrix-concept">${row.theological_concept}</span>
        </td>
        <td>${row.strategy}</td>
        <td><span class="matrix-badge">${row.moral_foundation}</span></td>
        <td>${row.redemptive_outcome}</td>
      `;
