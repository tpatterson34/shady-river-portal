/**
 * Our Heavy Load — Interactive Landmark & Jukebox App
 * Audio streaming engine, lyrics viewer, exhibits switcher, and release voting.
 */

(function () {
  'use strict';

  let data = window.HEAVY_LOAD_DATA || null;
  let tracks = [];
  ...
  function init() {
    if (!data && typeof window.HEAVY_LOAD_DATA !== 'undefined') {
      data = window.HEAVY_LOAD_DATA;
    }
  ...
  function renderTrackList() {
    ...
    const filtered = tracks.filter(function (t) {
      ...
      if (searchQuery) {
        const blob = (t.title + ' ' + t.summary + ' ' + t.cleanLyrics + ' ' + t.theory + ' ' + t.leadArchetype).toLowerCase();
    ...
    filtered.forEach(function (t) {
      ...
      row.className = 'track-row flex items-center justify-between p-3.5 sm:p-4 rounded-xl border transition-all cursor-pointer ' +
        (isCurrent ? 'active bg-orange-950/20 border-orange-500/50 text-white' : 'bg-stone-900/60 border-stone-800/80 hover:bg-stone-850 hover:border-stone-700 text-stone-300');
      row.setAttribute('data-track-index', idx);
      row.setAttribute('role', 'button');
      row.setAttribute('tabindex', '0');

      const iconHtml = (isCurrent && isPlaying)
        ? '<i class="fa-solid fa-volume-high text-orange-400 animate-pulse"></i>'
      ...
      row.innerHTML = [
        ...
        '  <span class="w-7 text-center font-mono text-xs ' + (isCurrent ? 'text-orange-400 font-bold' : 'text-stone-400') + '">',
        '    ' + iconHtml,
        '  </span>',
        '  <div class="min-w-0">',
        '    <div class="track-title font-display font-bold text-sm truncate ' + (isCurrent ? 'text-orange-300' : 'text-stone-200') + '">',
        '      ' + t.title,
        '    </div>',
        ...
        '  <span class="hidden md:inline-block px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider bg-stone-800/80 text-stone-400 border border-stone-700/50">',
        '    ' + t.leadArchetype,
        '  </span>',
        '  <button class="w-8 h-8 rounded-full ' + (isCurrent && isPlaying ? 'bg-orange-500 text-stone-950' : 'bg-stone-800 text-stone-300 hover:text-white') + ' flex items-center justify-center text-xs transition-colors" aria-label="Play ' + t.title + '">',
  ...
  function selectTrack(index, autoPlay) {
    ...
    if (trackTempoEl) trackTempoEl.textContent = track.tempo;
  ...
  function playAudio() {
    audio.play().then(function () {
      ...
      console.warn('Playback notice / user interaction needed:', err);
    });
  }
  ...
  function bindUIEvents() {
    ...
    if (cleanLyricsBtn) cleanLyricsBtn.addEventListener('click', function () {
      lyricsMode = 'clean';
      cleanLyricsBtn.classList.add('bg-orange-500', 'text-stone-950', 'font-bold');
      cleanLyricsBtn.classList.remove('bg-stone-800', 'text-stone-300');
      if (sunoLyricsBtn) {
        sunoLyricsBtn.classList.remove('bg-orange-500', 'text-stone-950', 'font-bold');
    ...
    if (sunoLyricsBtn) sunoLyricsBtn.addEventListener('click', function () {
      lyricsMode = 'suno';
      sunoLyricsBtn.classList.add('bg-orange-500', 'text-stone-950', 'font-bold');
      sunoLyricsBtn.classList.remove('bg-stone-800', 'text-stone-300');
      if (cleanLyricsBtn) {
        cleanLyricsBtn.classList.remove('bg-orange-500', 'text-stone-950', 'font-bold');
    ...
    actFilterBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        actFilterBtns.forEach(function (b) {
          b.classList.remove('bg-orange-600', 'text-stone-950', 'font-bold');
          b.classList.add('bg-stone-900', 'text-stone-300');
        });
        btn.classList.add('bg-orange-600', 'text-stone-950', 'font-bold');
  ...
  function submitVote() {
    localStorage.setItem('voted_our-heavy-load', 'true');
    checkVoteStatus();
    closeVoteModal();
  }

  function checkVoteStatus() {
    const voted = localStorage.getItem('voted_our-heavy-load');
    if (voteHeroBtn) {
      if (voted) {
        voteHeroBtn.innerHTML = '<i class="fa-solid fa-check text-emerald-400 mr-2"></i><span>Release Prioritized</span>';
        voteHeroBtn.classList.remove('bg-orange-600', 'hover:bg-orange-500', 'text-stone-950');
        voteHeroBtn.classList.add('bg-emerald-950/80', 'border', 'border-emerald-600/50', 'text-emerald-300', 'cursor-default');
      } else {
        voteHeroBtn.innerHTML = '<i class="fa-solid fa-vote-yea mr-2"></i><span>Vote to Release</span>';
      }
    }
  }
