/**
 * Red, White, and Robbed (The Ghost in the Server Room)
 * The Shady River Bard — 18 Studio Tracks, 4 Acts, Industrial-Techno Folk Opera
...
(function () {
  ...
  let data = window.RED_WHITE_ROBBED_DATA || null;
  let tracks = [];
  ...
  function init() {
    if (!data && typeof window.RED_WHITE_ROBBED_DATA !== 'undefined') {
      data = window.RED_WHITE_ROBBED_DATA;
    }
    if (data && data.tracks && data.tracks.length > 0) {
      ...
      fetch('data/album-data.json')
        ...
        .catch(function (err) {
          console.error('Failed to load Red, White, and Robbed album data:', err);
        });
    }
  }
  ...
  function renderTrackList() {
    ...
    filtered.forEach(function (t) {
      ...
      row.className = 'track-row flex items-center justify-between p-3.5 sm:p-4 rounded-xl border transition-all cursor-pointer ' +
        (isCurrent ? 'active bg-red-950/40 border-red-500/60 text-white shadow-lg shadow-red-950/20' : 'bg-[#0c1017] border-stone-800 hover:bg-[#131923] hover:border-stone-700 text-stone-300');
      ...
      row.innerHTML = [
        ...
        '  <span class="track-num w-7 text-center font-mono text-xs ' + (isCurrent ? 'text-red-400 font-bold' : 'text-stone-500') + '">',
        '    ' + iconHtml,
        '  </span>',
        '  <div class="min-w-0">',
        '    <div class="track-title-text font-display font-bold text-sm truncate ' + (isCurrent ? 'text-red-300' : 'text-stone-200') + '">',
        '      ' + t.title,
        '    </div>',
        ...
        '    ' + t.leadArchetype,
        '  </span>',
        '  <button class="w-8 h-8 rounded-full ' + (isCurrent && isPlaying ? 'bg-red-600 text-white font-bold' : 'bg-stone-800 text-stone-300 hover:text-white') + ' flex items-center justify-center text-xs transition-colors" aria-label="Play ' + t.title + '">',
    ...
    if (noteBtn) {
      noteBtn.onclick = () => {
        ...
        if (typeof window.openBardNoteModal === 'function') {
          window.openBardNoteModal(sTitle, "Red, White and Robbed", sNum);
        }
      };
  ...
  function bindUIEvents() {
    ...
    if (highContrastToggle) {
      highContrastToggle.addEventListener('click', function () {
        ...
        highContrastToggle.classList.toggle('text-red-400');
      });
    }
    if (largeFontToggle) {
      largeFontToggle.addEventListener('click', function () {
        document.body.classList.toggle('large-font');
        largeFontToggle.classList.toggle('text-red-400');
      });
    }
    if (underlineToggle) {
      underlineToggle.addEventListener('click', function () {
        document.body.classList.toggle('underline-links');
        underlineToggle.classList.toggle('text-red-400');
      });
    }
    if (reducedMotionToggle) {
      reducedMotionToggle.addEventListener('click', function () {
        document.body.classList.toggle('reduced-motion');
        reducedMotionToggle.classList.toggle('text-red-400');
      });
    }
  ...
  function submitVote() {
    localStorage.setItem('voted_red_white_robbed', 'true');
    checkVoteStatus();
    closeVoteModal();
  }

  function checkVoteStatus() {
    const voted = localStorage.getItem('voted_red_white_robbed');
    if (voteHeroBtn) {
      if (voted) {
        voteHeroBtn.innerHTML = '<i class="fa-solid fa-check text-emerald-400 mr-2"></i><span>Release Prioritized</span>';
        voteHeroBtn.classList.remove('bg-red-600', 'hover:bg-red-500', 'text-white');
