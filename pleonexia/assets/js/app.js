/**
 * Pleonexia — Interactive Concept Album Engine & Audio Jukebox
 * The Shady River Bard — 13 Studio Tracks, 4 Movements, Pure Literary Lyrics
 */

(function () {
  'use strict';

  let data = window.PLEONEXIA_DATA || null;
  let tracks = [];
  ...
  function init() {
    if (!data && typeof window.PLEONEXIA_DATA !== 'undefined') {
      data = window.PLEONEXIA_DATA;
    }
    if (data && data.tracks && data.tracks.length > 0) {
      ...
      fetch('data/album-data.json')
        ...
        .catch(function (err) {
          console.error('Failed to load Pleonexia album data:', err);
        });
    }
  }
  ...
  function renderTrackList() {
    ...
    filtered.forEach(function (t) {
      ...
      row.className = 'track-row flex items-center justify-between p-3.5 sm:p-4 rounded-xl border transition-all cursor-pointer ' +
        (isCurrent ? 'active bg-sky-950/30 border-sky-500/60 text-white' : 'bg-[#0d121b] border-stone-800 hover:bg-[#131a26] hover:border-stone-700 text-stone-300');
      row.setAttribute('data-track-index', idx);
      row.setAttribute('role', 'button');
      row.setAttribute('tabindex', '0');

      const iconHtml = (isCurrent && isPlaying)
        ? '<i class="fa-solid fa-volume-high text-sky-400 animate-pulse"></i>'
      ...
      row.innerHTML = [
        ...
        '  <span class="track-num w-7 text-center font-mono text-xs ' + (isCurrent ? 'text-sky-400 font-bold' : 'text-stone-500') + '">',
        '    ' + iconHtml,
        '  </span>',
        '  <div class="min-w-0">',
        '    <div class="track-title-text font-display font-bold text-sm truncate ' + (isCurrent ? 'text-sky-300' : 'text-stone-200') + '">',
        '      ' + t.title,
        '    </div>',
        ...
        '    ' + t.leadArchetype,
        '  </span>',
        '  <button class="w-8 h-8 rounded-full ' + (isCurrent && isPlaying ? 'bg-sky-500 text-stone-950 font-bold' : 'bg-stone-800 text-stone-300 hover:text-white') + ' flex items-center justify-center text-xs transition-colors" aria-label="Play ' + t.title + '">',
    ...
    if (noteBtn) {
      noteBtn.onclick = () => {
        ...
        if (typeof window.openBardNoteModal === 'function') {
          window.openBardNoteModal(sTitle, "Pleonexia", sNum);
        }
      };
  ...
  function bindUIEvents() {
    ...
    actFilterBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        actFilterBtns.forEach(function (b) {
          b.classList.remove('bg-sky-600', 'text-stone-950', 'font-bold');
          b.classList.add('bg-stone-900', 'text-stone-300');
        });
        btn.classList.add('bg-sky-600', 'text-stone-950', 'font-bold');
    ...
    if (highContrastToggle) {
      highContrastToggle.addEventListener('click', function () {
        ...
        highContrastToggle.classList.toggle('text-sky-400');
      });
    }
    if (largeFontToggle) {
      largeFontToggle.addEventListener('click', function () {
        document.body.classList.toggle('large-font');
        largeFontToggle.classList.toggle('text-sky-400');
      });
    }
    if (underlineToggle) {
      underlineToggle.addEventListener('click', function () {
        document.body.classList.toggle('underline-links');
        underlineToggle.classList.toggle('text-sky-400');
      });
    }
    if (reducedMotionToggle) {
      reducedMotionToggle.addEventListener('click', function () {
        document.body.classList.toggle('reduced-motion');
        reducedMotionToggle.classList.toggle('text-sky-400');
      });
    }
  ...
  function submitVote() {
    localStorage.setItem('voted_pleonexia', 'true');
    checkVoteStatus();
    closeVoteModal();
  }

  function checkVoteStatus() {
    const voted = localStorage.getItem('voted_pleonexia');
    if (voteHeroBtn) {
      if (voted) {
        voteHeroBtn.innerHTML = '<i class="fa-solid fa-check text-emerald-400 mr-2"></i><span>Release Prioritized</span>';
        voteHeroBtn.classList.remove('bg-sky-600', 'hover:bg-sky-500', 'text-stone-950');
